...
- **I test vengono eseguiti insieme alla modifica del codice**; 995 test hanno avuto esito positivo grazie al lavoro di due persone, con una pipeline CI basata su percorsi per i 950 test più importanti.
...
CI esegue il sottoinsieme ermetico della suite su **ubuntu-latest / Python 3.12** con installazioni bloccate (`.github/workflows/ci.yml`); la fase di creazione degli artefatti richiede le strutture registrate in `E:\AI\training`, che non sono presenti in Git, quindi CI le esclude intenzionalmente. Localmente, `python -m pytest` esegue tutti i **995** test e `python -m pytest -m "not artifacts"` esegue gli **950** test riprodotti da CI.
