...
**Quattro asset accettati, appartenenti a quattro classi di soggetti diversi, senza costi aggiuntivi.** Ognuno è stato valutato dal Direttore con il proprio livello di zoom: sul file GLB o su fogli a grandezza naturale, e non in base a una metrica che supera una soglia.
...
| **Galleon** | veicolo, con struttura leggera | [2026-08-05](docs/experiments/E04-ruling.md) | 36.89 / 6.87 / 56.24 |
...
Le condivisioni si basano su texel validi e **non sono confrontabili tra soggetti diversi**: una nave nasconde la maggior parte di sé dalla prospettiva a livello degli occhi e un animale ne nasconde metà. Valuta ciascuno rispetto al proprio limite di copertura pre-registrato, rispetto al quale raggiungono l'**86–93%**: la differenza tra le righe è la geometria, non una regressione. [Numeri completi, con i rispettivi
denominatori](docs/handbook/subjects.md).

**È una pipeline, non un generatore di un singolo elemento.** Contraddici le specifiche su otto elementi specifici e la richiesta avrà successo in **8 casi su 8**: la deviazione mediana ΔE è di 46,3 rispetto a 6,2 su cinque controlli mantenuti; nel frattempo, l'aspetto rimane lo stesso. La struttura è mantenuta dalla mesh e dal controllo; gli attributi specificati influenzano la richiesta.
...
Sei risultati, ognuno dei quali ha richiesto un esperimento e ognuno dei quali si generalizza oltre il soggetto che lo ha prodotto. [La versione estesa, con le
...
- **Chiedi informazioni sulla geometria, non su una soglia.** La sostituzione di una maschera chiave con l'esatta sagoma ottenuta tramite raycast ha aumentato la copertura del riferimento dal **28,4% al 39,1%** dei texel validi: questo è strettamente additivo, senza diffusione e senza utilizzo della GPU. Il keying angolare-mediano ha fallito tre volte in questa fase ed è stato abbandonato.
- **Elimina ciò che nessuna telecamera può vedere, dall'atlante e mai dalla mesh.** Il 49% dei texel dell'atlante sono invisibili dall'esterno; l'esclusione di queste facce riduce l'interpolazione del 68%. L'esclusione anziché l'eliminazione rende il fallimento impossibile invece che semplicemente rilevabile.
...
- **La fascia della lama occupa lo 0,00% del riferimento nella fase 1** su tutte e otto le telecamere: l'acciaio su uno sfondo grigio si trova esattamente sulla soglia del key. L'unione salva il 55,72%.
- **Le giunture delle texture non sono uniformi.** Un confine di provenienza crea una variazione della texture pari a **5,5 volte** quella ordinaria; la regione specificata dal Direttore crea una variazione pari a **9,5 volte**.
...
La disciplina è tanto importante quanto la pipeline stessa, ed esiste per un motivo: una fase precedente ha eseguito dieci sessioni in cui ogni output è stato valutato e le conclusioni sono state scritte per essere lette nella sessione successiva come fatti consolidati. Nulla in quel ciclo era verificabile.

- **Definire le specifiche prima del lavoro, redigere un rapporto dopo e stabilire una decisione finale:** la sessione che progetta un esperimento non valuta mai i propri risultati. Ventuno esperimenti sono disponibili in [questa sezione](docs/experiments/).
- **Le correzioni vengono applicate immediatamente, accanto alla misurazione che le ha invalidate**, e non come semplici eliminazioni. Sei affermazioni precedenti sono state confutate nella sessione iniziale e tutte e sei sono ancora visibili accanto a ciò che le ha sostituite.
...
- **I test sono eseguiti insieme al commit che modifica il codice:** 248 superati con due persone che lavorano, con CI basata sui percorsi per i 240 elementi ermetici.
- **La cronologia è consultabile.** Un indice SQLite + FTS5 sull'intera sequenza, verificato su quattro livelli. Ha individuato un numero di risultati diversi da quello indicato nel testo in tre posizioni, contando direttamente la cronologia.
...
| **[La cronologia](docs/experiments/)** | ventuno esperimenti: specifiche, rapporto, decisione finale e ogni previsione dichiarata prima della misurazione. |
...
| **[L'evoluzione, come si è svolta](docs/arc-history.md)** | la cronologia, con le correzioni intatte. |
...
- **Dati NON interessati:** nessuna credenziale, mai. Nessuno strumento legge, memorizza o trasmette token, chiavi o password, e nessuno di questi è presente nell'albero: sono stati eseguiti controlli per individuare chiavi con prefisso del provider, GitHub PAT, token Slack, ID chiave AWS, blocchi di chiavi private, token bearer e assegnazioni inline `api_key`/`password`; **zero corrispondenze**, nessun file che assomigli a una credenziale è stato rilevato.
- **Nessun telemetria.** Nessuna raccolta o trasmissione di dati. Non c'è alcuna opzione per disattivare la funzionalità perché non c'è nulla da disattivare.
- **Traffico di rete:** due degli strumenti su trentaquattro aprono una connessione socket: `restylize_views.py` e `texpass_brush.py`, entrambi chiamano un'API HTTP di ComfyUI all'indirizzo `--host`, con valore predefinito `127.0.0.1:8188`. Nessun altro strumento in `tools/` effettua chiamate di rete.
...
Tre aspetti critici vengono evidenziati anziché nascosti, perché una nota sulla sicurezza che elenca solo le rassicurazioni non è un modello di minacce: **le operazioni sui file non sono eseguite in un ambiente isolato** (uno strumento scrive ovunque gli indichino i suoi argomenti); **molti strumenti e documenti contengono percorsi locali assoluti**, 114 occorrenze in 26 file, non segreti ma una divulgazione del layout di una macchina e il motivo per cui la maggior parte degli strumenti non funzionerà se modificati su un'altra macchina; e **i fallimenti imprevisti si manifestano come tracce di errore Python nei 34 script di ricerca non pubblicati**, senza alcun filtro `--debug`. Le interruzioni deliberate sono messaggi `ANDON:` che contengono la misurazione che le ha attivate. Questo è il contratto dello strumento di ricerca e [SHIP_GATE.md](SHIP_GATE.md) registra esattamente quando smette di essere sufficiente, cosa che per i due comandi che facet *installa* è avvenuta alla versione 0.2.0: `facet-index` e `facet-mcp` restituiscono `0` ok / `1` errore utente / `2` errore di runtime e rifiutano l'esecuzione con un messaggio di errore strutturato che indica il passaggio successivo anziché una traccia ([E21](docs/experiments/E21-cli-contract-report.md)).

**Stato del supporto:** questo repository viene sviluppato in modo aperto, su un'unica macchina, da un unico responsabile e da sessioni di consulenza ed esecuzione a rotazione. `main` è l'unico stato supportato. Non esiste un canale di rilascio, una politica di backport o un SLA: ciò che esiste è la cronologia: ogni affermazione si trova accanto al codice che la produce e [docs/experiments](docs/experiments/) contiene le specifiche, il rapporto e la decisione finale per ciascuna.
...
CI esegue il sottoinsieme ermetico della suite su **ubuntu-latest / Python 3.12** con installazioni bloccate (`.github/workflows/ci.yml`); la fase di creazione degli artefatti richiede le strutture registrate in `E:\AI\training`, che non sono presenti in Git, quindi CI le esclude intenzionalmente. Localmente, `python -m pytest` esegue tutti i **248** test e `python -m pytest -m "not artifacts"` esegue le **240** ripetizioni di CI.
