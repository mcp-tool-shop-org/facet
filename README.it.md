...
⚠ **`pip install facet-mcp` era difettoso in tutte le versioni rilasciate fino alla v0.3.0, ed è stato corretto nella v0.3.1.** Il pacchetto installa `facet_index` come modulo di livello superiore; quindi, fino alla versione v0.3.0 inclusa, individuava la posizione del record rispetto a `<venv>/Lib` (che non contiene né il corpus né l'indice) e `build`, `claims` e `q` senza `--db`, tutti i quali fallivano.
**Nelle versioni v0.3.0 o precedenti, utilizza il file binario `npx` indicato sopra.**

Dalla versione v0.3.1, la radice viene risolta **verificando l'esistenza del record** anziché presupponendola: esegui uno qualsiasi dei due comandi all'interno di una copia del progetto e il sistema lo troverà; eseguilo da qualsiasi altra posizione e restituirà **`4` RIFIUTATO**, indicando sia le directory che ha provato a utilizzare, sia i marcatori che ha cercato.
`$FACET_INDEX_DB` viene ora letto da entrambi i comandi e seleziona quale *indice* utilizzare, non quale *corpus*. I risultati sono stati misurati su un pacchetto creato da `main` e installato in un ambiente virtuale pulito: [E24](docs/experiments/E24-ruling.md).
...
Successivamente, affermava che il pacchetto "funziona solo per `q` e `claims`" - **anche `claims` non funzionava**, come scoperto eseguendolo nell'esperimento E24. Entrambe le correzioni sono presenti in [known-defects.md](docs/known-defects.md) con i relativi risultati.*

## Stato attuale

**Quattro asset accettati, appartenenti a quattro classi di soggetti diversi, senza costi aggiuntivi.** Ognuno è stato valutato dal direttore utilizzando il proprio livello di zoom (sulla mesh GLB o su fogli di dimensioni reali), e non tramite una metrica che supera una soglia.
...
Le condivisioni si riferiscono a texel validi e **non sono comparabili tra soggetti diversi**: una nave nasconde la maggior parte di sé dalla prospettiva dell'osservatore, mentre un animale ne nasconde metà. Valuta ogni elemento rispetto al proprio limite di copertura pre-registrato; in questo modo, i risultati si attestano nell'intervallo **86–93%**: la differenza tra le righe è dovuta alla geometria, non a una regressione. [Numeri completi con i relativi denominatori](docs/handbook/subjects.md).

**Si tratta di una pipeline, non di un generatore che produce un singolo elemento.** Contradici le specifiche su otto elementi nominati e la richiesta avrà successo in **8 casi su 8**: la mediana ΔE è pari a 46,3 rispetto a 6,2 su cinque controlli mantenuti; nel frattempo, l'aspetto dell'immagine rimane invariato. La struttura è garantita dalla mesh e dal controllo; gli attributi nominati vengono gestiti tramite la richiesta.
...
Fase per fase, con le relative motivazioni: **[la guida](docs/handbook/index.md)**.

## Cosa rende il sistema efficace

Sei risultati, ciascuno dei quali ha richiesto un esperimento e ciascuno dei quali è generalizzabile oltre il soggetto che lo ha prodotto. [La versione completa con i risultati](docs/findings.md).
...
- **Definisci il volto, ottieni un volto.** Un ritaglio che inquadra il busto aggiunge **da 3,1 a 4,5 volte più poligoni alla testa**, e la differenza è strutturale: palpebre separate, una piega sul sopracciglio, cavità delle narici modellate; non si tratta semplicemente di una sfocatura meno marcata.
...
- **L'identità appartiene alla richiesta.** Un elemento canonico non nominato nella richiesta viene aggiunto accidentalmente e scomparirà allo stesso modo: questo è stato misurato quando si è scoperto che delle placche dorate sulle ginocchia apparivano nell'immagine solo a causa del rumore in un ControlNet difettoso.
...
- **Le giunture delle strisce non sono livellate.** Un confine di provenienza presenta una variazione della trama **5,5 volte** superiore a quella ordinaria; la regione nominata dal direttore presenta una variazione **9,5 volte** superiore.
...
- **Ogni ricostruzione in questo percorso è un guscio cavo a doppia parete**, le pareti misurano circa due voxel. Nessun predicato volumetrico è valido su uno di essi.
...
- **Definizione prima del lavoro, relazione dopo, decisione finale:** e la sessione che progetta un esperimento non valuta mai i propri risultati. Ventisei esperimenti sono disponibili in [questa sezione](docs/experiments/).
- **Le correzioni vengono inserite al loro posto, accanto alla misurazione che le ha confutate**, e non come semplici eliminazioni. Solo nella sessione iniziale sono state falsificate sei affermazioni ereditate, e tutte e sei sono ancora leggibili accanto a ciò che le ha sostituite.
...
- **I test sono associati al commit che modifica il codice:** 797 superati da due persone, con CI basata sui percorsi per i 768 elementi ermetici.
...
| **[La sezione](docs/experiments/)** | ventisei esperimenti: definizione, relazione, decisione e ogni previsione indicata prima della misurazione |
...
facet viene eseguito interamente sulla propria macchina: ogni strumento è uno script che si esegue su percorsi digitati, quindi la domanda utile non è *quali autorizzazioni richiede questa app*, ma *cosa fanno questi script alla tua macchina*. La risposta è fornita dalla misurazione, con ogni ciclo ripetibile; la politica completa è disponibile in [SECURITY.md](SECURITY.md):
...
- **Traffico di rete:** due degli strumenti su trentaquattro aprono una socket: `restylize_views.py` e `texpass_brush.py`, ed entrambi chiamano l'API HTTP di ComfyUI all'indirizzo `--host`, con valore predefinito `127.0.0.1:8188`. Nient'altro in `tools/` effettua una chiamata di rete.
- **Autorizzazioni:** utente ordinario. Nessun aumento dei privilegi, nessuna installazione del servizio, nessuna scrittura nelle impostazioni di sistema o nel registro di sistema.

Tre spigoli vivi vengono evidenziati anziché eliminati, perché una nota di sicurezza che elenca solo garanzie non è un modello di minaccia: **le operazioni sui file non sono eseguite in un ambiente isolato** (uno strumento scrive ovunque indichino i suoi argomenti); **i percorsi locali assoluti sono incorporati in molti strumenti e documenti** — 114 occorrenze in 26 file, non segreti ma una divulgazione della struttura di una macchina e la ragione per cui la maggior parte degli strumenti non funzionerà senza modifiche altrove; e **gli errori imprevisti si manifestano come tracce di Python nei 34 script di ricerca non pubblicati**, senza un filtro `--debug`. Le interruzioni intenzionali sono messaggi `ANDON:` che contengono la misurazione che le ha attivate. Questo è il contratto dello strumento di ricerca e [SHIP_GATE.md](SHIP_GATE.md) registra esattamente quando smette di essere sufficiente, cosa che per i due comandi che lo installano, a partire dalla versione 0.2.0: `facet-index` e `facet-mcp` restituiscono `0` ok / `1` errore utente / `2` errore in fase di esecuzione — e, come indicato in [E22](docs/experiments/E22-ruling.md), **`4` RIFIUTATO** per un filtro attivato o una sezione `verify` che non funziona, il che significa che lo strumento sta funzionando e ti avvisa di non procedere anziché generare un errore in fase di esecuzione. Tutti rifiutano con un messaggio di errore strutturato che indica il passaggio successivo anziché una traccia ([E21](docs/experiments/E21-cli-contract-report.md)).
...
Il CI esegue il sottoinsieme ermetico della suite su **ubuntu-latest / Python 3.12** con installazioni fisse (`.github/workflows/ci.yml`); lo strato degli artefatti richiede gli alberi registrati in `E:\AI\training`, che non sono presenti in git, quindi il CI li esclude intenzionalmente. Localmente, `python -m pytest` esegue tutti i **797** test e `python -m pytest -m "not artifacts"` esegue i **768** test riprodotti dal CI.
