...
- **テストは、コードに触れるコミットと連携します。** 2つの環境で648件のテストが合格し、640件の隔離されたテストに対してパスベースのCIが実行されます。
...
CIは、**ubuntu-latest / Python 3.12**上で、スイートの隔離されたサブセットを実行し、ピン留めされたインストールを行います（`.github/workflows/ci.yml`）。アーティファクト層には、記録されたツリーが必要です（`E:\AI\training`）。これらはgitには含まれていないため、CIは設計によりそれらを非選択にします。ローカルでは、`python -m pytest`がすべての**648個のテスト**を実行し、`python -m pytest -m "not artifacts"`がCIで再現される**640個のテスト**を実行します。
