...
- **Las pruebas están vinculadas al commit que modifica el código**: 1068 superaron la prueba con la participación de dos personas, con CI restringida por rutas en las 1023 pruebas herméticas.
...
CI ejecuta el subconjunto hermético de la suite en **ubuntu-latest / Python 3.12** con instalaciones fijas (`.github/workflows/ci.yml`); la capa de artefactos necesita los árboles registrados que se encuentran en `E:\AI\training`, y que no están en Git, por lo que CI los excluye intencionadamente. Localmente, `python -m pytest` ejecuta las **1068** pruebas y `python -m pytest -m "not artifacts"` ejecuta las **1023** pruebas que reproduce CI.
