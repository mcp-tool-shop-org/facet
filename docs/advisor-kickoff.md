...
| suite | **THE SUITE: 1339 tests, 1285 hermetic.** RE-COUNT before quoting — `pytest --collect-only -q` and again with `-m "not artifacts"`, currently 1339 total / 1285 hermetic. T34 pins ~14 count surfaces **and** a separate set of experiment-count surfaces read off the status table in `docs/experiments/README.md` — that table is the authority, not the index. ⚠ **T34 also pins three anchor SENTENCES in this file**; rewrite one and it fires, and one of them is the fixture a can-fail leg rewrites by hand, so restore the phrasing verbatim rather than re-pinning it |
| the record | four-leg verify SERVING at close, byte-identity, over 60 experiments. No staleness findings at the declaration leg — recertify with MCP `record_build` after any doc change. 415 corpus files; `paid_for_by` reports 0 unrecognised |
