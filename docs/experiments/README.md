...
| [E08](E08-cover-the-figure-with-reference.md) | Can the styled reference cover the figure — and if it does, does the defect the Director rejected go with it? 28.4% of the asset is reference; the two-view limit is a hardcoded list, not a property of the route. | **SPEC — highest priority.** Gate 0 is free and has two halts: the geometric ceiling at N = 2/4/6/8/12 cameras (halt below 60% at 8), and **the metric's own validation against the rejected asset** — the direct remediation for E07. Grades on hold-one-out reference agreement in **ΔE**, an external perceptual constant, not a high-pass statistic |
