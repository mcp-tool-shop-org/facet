...
| [E07](E07-the-atlas-is-not-a-neighbourhood.md) | The loop writes colour from two places and neither knows where a texel's neighbours are *on the surface*. Does giving it one remove the defect class the Director named? | **RULED at Gate 0.5** → [Gate 0](E07-gate0.md), [Gate 0.5](E07-gate0_5.md), [Amendments 1-2](E07-the-atlas-is-not-a-neighbourhood.md#0-rules). L1's ANDON measured a **proxy** — normal disagreement — where the failure mode is *source distance*; the back-facing class turned out to be the **closest** class (0.77 edges) and 0.00% of lookups reach beyond 20 edges. Gate restated in triangle edges, hemisphere variant rejected, **L1 authorised**. L2's pass condition **withdrawn as mis-formed** (fourth in the ledger — a fraction of a baseline nobody had measured); GPU **not** authorised until the membrane is re-anchored on the accepted set rather than the job-mask contour. Both premises measured and neither halt fired: **74.9%** of dilated texels take colour from another island, from a median **0.177 away on a figure 1.0 tall**; the cross-provenance step ratio is **5.500**. **L1's own ANDON fired** — 48.67% of surface-aware lookups source from a back-facing normal against a 20% limit, and those sources are the *closest* class (0.77 edges, 66.7% within one triangle). L2's offline bound moved the ratio 5.500 → 5.000 against a ≤3.25 pass line, passing the flattening guard at −0.62%. Correction to the spec: **the gutter is not the mechanism** — only 32.5% of paths cross one, and the minimal `& valid` patch still leaves 53.3% cross-island while stranding 174,898 texels |
