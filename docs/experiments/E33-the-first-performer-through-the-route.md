...
| 9 | The **saltroad LoRA is on Comfy Cloud** | **MEASURED FALSE — see §"The LoRA is not there".** `search_models` returns **0 rows** for `q="saltroad"` `type="lora"` and **0 rows** for `q="<operator>"`. The same probe returns exactly one row for each base model above, so the query form is precise |
