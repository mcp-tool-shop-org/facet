...
| `prop.json` | SWEPT; generation keys SPENT at the accepted pair; the brush block CLEARED | canny 0.10/0.25 and the backdrop RULED (E14 Rulings 6/7); elevated NONE (Ruling 8); the recipe anchored on the accepted pair (Ruling 16b — seed reads SPENT, two-seed fact recorded); the palette wired at `_fixtures.palette` (Ruling 17, report-only); the stroke lane RULED at Ruling 24 — `texpass_brush` cleared (eight merged strokes, spiral order, the garnet demotion with its named compensator, stems v3) and `thin-extent` DECIDED at 0.0 (Ruling 24d — the deferral pattern held to its end and closed with the 10d curve in the room) |
