...
| `reconstruct_mesh.py` | image-to-3D through TRELLIS.2 **at a chosen seed** — the recorded call path with `seed=` added, plus a sidecar recording every parameter | added at E37: `pipe.run` has always taken `seed: int = 42` and neither runner on this rig passed it, so every recorded reconstruction here ran at 42 by default and seed variation was unaskable. Anchored at `--anchor clay`, which reproduces E29's recorded raw counts **2,208,416 v / 4,430,096 f exactly** (E29 Ruling 5: the raw stage is bit-identical, `to_glb` is not). Its seed gate refuses by introspection before the model loads, because a signature that accepted and ignored a seed would return N identical meshes under N names. T71 |
