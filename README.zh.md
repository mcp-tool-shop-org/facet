...
- **测试与修改代码相关的提交版本相关联**——在两次操作中，有 1087 个通过测试，并且对 1042 个经过严格隔离的版本进行了路径限制的 CI 测试。
...
CI 在 **ubuntu-latest / Python 3.12** 上运行套件的封闭子集，并进行固定安装（`.github/workflows/ci.yml`）；工件层需要记录在 `E:\AI\training` 下的树，这些内容不在 git 中，因此 CI 会按设计选择不包含它们。本地，`python -m pytest` 运行所有 **1087** 个测试，而 `python -m pytest -m "not artifacts"` 运行 CI 重现的 **1042** 个测试。
