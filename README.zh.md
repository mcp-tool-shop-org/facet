...
- **测试与修改代码相关的提交一起进行**——在两个人的操作下，有 1295 个测试通过，并且对 1241 个密封性测试进行了路径限制的 CI 测试。
...
CI 在 **ubuntu-latest / Python 3.12** 上运行套件的隔离子集，并进行固定安装（`.github/workflows/ci.yml`）；工件层需要记录在 `E:\AI\training` 下的树，这些内容不在 git 中，因此 CI 会按设计选择不包含它们。本地，`python -m pytest` 运行所有 **1295** 个测试，而 `python -m pytest -m "not artifacts"` 运行 CI 重现的 **1241** 个测试。
