...
这种风格应用于**资源本身**，在纹理空间中进行处理——而不是针对每个视角单独绘制，然后再将它们拼接在一起。只需向程序输入一个外形夸张的泥塑概念图，它就会生成一个带有纹理的模型，该模型的颜色来自对该模型进行过风格化的参考图像，而参考图像无法显示的区域则会通过蒙版修复画笔和具有表面感知的膨胀功能来填充。

这个名称既指出了问题的两个方面：多边形，以及它们所构成的表面。

## 目前的状况如何？

**四种被批准的素材，分别属于四个不同的主题类别，且不计入学分。** 主任对每一种素材都进行了评估，他会根据自己的判断（是在 GLB 上，还是在全尺寸纸张上）来决定是否采用，而不是简单地以某种标准作为衡量依据。

| 主题；学科；对象；使……屈从 | 类 | 接受；被接纳。 | 参考；刷子；扩张 |
|---|---|---|---|
| **Character (W3)** | 人形的；类似人类的。 | [2026-08-04](docs/experiments/E08-ruling-gate0.md) | 68.8 / 4.2 / 27.0 |
...
这些样本都是有效的纹素，并且**它们不能在不同对象之间进行比较**——一艘船会隐藏其大部分结构，使其低于观察者的视线；而动物则只隐藏一半身体。将每个样本与其预先设定的最大可见范围进行对比，结果显示它们达到该范围的**86%–93%**：行与行之间的差异在于几何形状，而不是回归分析。[完整的数值及其分母](docs/handbook/subjects.md)。

**这是一个流水线，而不是一个简单的单字符生成器。** 它与八个指定元素的规范相悖，但生成的图像却获得了 **8/8** 的好评——中位 ΔE 值为 46.3，而五个对照组的平均值为 6.2——同时，图像中的人物保持不变。结构由网格和控制机制维持；指定的属性则受到提示词的影响。
...
- **孪生模型属于网格，而非角色。** 在不同的网格中重复使用孪生模型，可以使覆盖率从 **62% 降低到 22.7%**，因为手臂会投射到模型旁边的空白区域。每次在进行纹理绘制时，都应根据当前的网格生成孪生模型。
- **身份归属于提示词。** 如果提示词中没有提及的经典元素意外出现，它也会以同样的方式消失——例如，当金色膝盖护甲仅通过损坏的 ControlNet 中的噪点出现在图像中时，我们就可以衡量其影响。
- **询问几何形状，而不是阈值。** 将关键遮罩替换为精确的光线投射轮廓，可以将参考覆盖率从 **28.4% 提高到 39.1%** 的有效纹素——完全是累加式的，没有扩散，也不需要 GPU。角落中值抠图在这里已经失败了三次，现在已被弃用。
- **剔除相机无法看到的区域，从贴图中剔除，而不是从网格中删除。** 49% 的贴图纹素在外部不可见；排除这些面可以使插值减少 **68%**。与其删除，不如排除，这样可以将失败的可能性完全消除，而不仅仅是使其可被检测到。

## 还有哪些问题没有得到解决？

这些问题都已明确列出并进行了说明，而且是在文档的首页上，而不是在脚注中。[所有问题都位于代码文件中](docs/known-defects.md)。

- 在所有八个摄像头中，**刀片带占据了第一阶段参考图像的 0.00%**——在灰色背景上的一块金属正好位于关键点的阈值位置。这种融合方式使画面质量提升了 55.72%。
- **笔触边缘没有进行平滑处理。**一个来源边界呈现出**5.5倍**于普通纹理变化的程度；导演所指的区域则呈现出**9.5倍**的变化。
- **膨胀效果导致不相关的图块之间出现颜色溢出**——74.9%的膨胀像素从另一个图块获取颜色，平均距离为0.177（在高度为1.0的图像中）。
- **这条路径上的每一次重建都形成了一个空心的双层外壳**，外壳厚度约为两个体素。无法对单个外壳应用任何体积谓词。

## 这个代码仓库是如何运行的？

这种方法既是结果，也是过程，它存在的目的是：在之前的流程中，进行了十次评估，每次都对自己的成果进行判断，并在下一次会议中将这些判断作为既定事实进行讨论。在这个循环中，没有任何环节可以被验证。

- **先制定计划，后进行报告，最后得出结论**——而且，设计实验的环节绝不会对自身的结果进行评估。目前已有二十个实验记录在[此处](docs/experiments/)。
- **更正会直接显示在相应的位置，与它所推翻的测量结果并列**，而不是以静默删除的方式呈现。仅在最初的环节中，就有六项既有主张被证明是错误的，而且这六项主张仍然可以与替代它们的内容一起查看。
- **失败的结果会保留在代码库中，并附带其原因。**[`tools/superseded/`](docs/tools.md) 并非一个存档——任何人都可以运行这些工具，并观察它们以相同的方式失败。
- **负面结果也是一种完整的成功**，它会被报告和记录下来，而不是被调整到某个特定数值。
- **测试会与修改代码的提交一起进行**——共有218个测试通过，由两位开发者共同完成，并且对其中210个进行了路径限制的持续集成（CI）。
- **可以查询记录。**针对整个过程建立了 SQLite + FTS5 索引，并在四个方面进行了验证。它通过统计记录本身，发现了三个地方的文本描述与实际结果不符的情况。
...
| **[记录](docs/experiments/)** | 二十个实验：规范、报告、结论，以及在测量之前声明的每一个预测。 |
...
有意识地排除，并说明原因：**nvdiffrast**（非商业用途——通过结构性触发机制强制执行，而非通过认证），**Hunyuan3D-Paint**（在欧盟、英国和韩国的许可无效）、**MVPaint** 和 **TEXGen**（完全没有许可），以及 **UltraSharp / SUPIR / StableSR**（非商业用途的图像增强工具）。
...
公开了三个关键点，而不是声称已经解决了它们，因为仅列出保证的安全说明并不是一个威胁模型：**文件操作没有进行沙箱保护**（工具会在其参数指示的位置写入）；**许多工具和文档中都嵌入了绝对本地路径**——在 26 个文件中共有 114 处出现，这并非秘密，而是对一台机器的布局的披露，也是大多数工具无法在其他地方未经修改运行的原因；并且 **意外故障会以 Python 堆栈跟踪的形式显示**，没有 `--debug` 网关，也没有结构化的错误形式。有意的停止操作是 `ANDON:` 消息，其中包含触发它们的测量结果。这就是研究工具的约定，[SHIP_GATE.md](SHIP_GATE.md) 记录了何时不再满足要求。

**支持状态：**此仓库在一个环境中进行开放式开发，由一位负责人和一个轮流出现的顾问和执行者团队负责。`main` 是唯一受支持的状态。没有发布渠道、回溯策略或 SLA——取而代之的是记录：每个声明都与其生成代码相邻，并且 [docs/experiments](docs/experiments/) 包含每个实验的规范、报告和结论。
...
CI 在 **ubuntu-latest / Python 3.12** 上运行该套件的隔离子集，并使用固定的安装（`.github/workflows/ci.yml`）；工件层需要记录的树，位于 `E:\AI\training` 中，这些内容不在 git 中，因此 CI 会按设计选择排除它们。在本地，`python -m pytest` 运行所有 **218** 个测试，并且 `python -m pytest -m "not artifacts"` 运行 **210** 个 CI 重现的测试。
