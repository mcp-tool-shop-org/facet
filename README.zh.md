...
- **测试会与触及代码的提交一起进行**——在两个人的手中，有 1053 个通过测试，并且对 1008 个封闭的测试进行了路径门控 CI。
...
在本地环境中，`python -m pytest` 将运行所有 **1053** 个测试用例，而 `python -m pytest -m "not artifacts"` 则会运行 **1008** 个 CI 重现的测试用例。
