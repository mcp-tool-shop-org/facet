...
- **测试与修改代码的代码提交相关联**——在两个人的操作下，有648个测试通过，并且对这640个经过严格测试的测试进行了路径限制的CI（持续集成）。
...
本地，`python -m pytest` 运行所有 **648 个** 测试，而 `python -m pytest -m "not artifacts"` 运行 CI 重现的 **640 个** 测试。
