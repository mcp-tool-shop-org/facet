...
这八个图像的合成方式如下：从基于边界×朝向×可见性权重的逐视图束调整中重建，然后渲染到图集中。导演认为其效果“好多了”，并且“看起来很棒”——与已发布的图集相比，该图集的渲染流程曾导致图像出现问题。完成此过程的链条位于`tools/`（`emit_view_aovs`、`s3_composite`、`flow_estimate`、`s3_run`、`s3_sheet`、`atlas_from_aovs`、`twin_mesh_warp`），其中五个由外部通道构建。这些通道指定的校准参数已通过验证，并且“十次尝试都成功”，每次都在信任之前进行验证。剩余部分如下所示，不再隐藏：正在调查的填充多边形类、等待策略的新表面以及导演认为至关重要的标准构建。
...
- **某些可见表面映射到图集中，但任何烘焙过程都不会写入这些表面**，因此渲染出的图像呈现出未修改的默认黑色。Blender的烘焙工具使用纹素中心采样，因此没有与任何纹素中心重叠的三角形将保持空白——其开发者已命名该机制并合并了修复程序（[https://projects.blender.org/blender/blender/pulls/161752](https://projects.blender.org/blender/blender/pulls/161752)），此修复程序是在测量此处所有数值的两周后合并的。这是一个与渲染流程相关的属性，而不是特定对象：在一个资源上进行测量，在其他四个资源上未进行测量。
- **刀片带占所有八个摄像机中第一阶段参考图像的0.00%**——钢材放置在灰色背景上，正好位于关键阈值位置。该组合修复了55.72%。
- **笔触缝隙没有对齐。**一个来源边界与普通纹理变化相比，差异为**5.5倍**；导演命名的区域的差异为**9.5倍**。
- **膨胀会导致不相关的图集岛屿之间出现混叠**——74.9%的膨胀纹素从另一个岛屿获取颜色，与平均距离为1.0（在高度为1.0的图像上）的岛屿相距0.177。⚠ **该比例是在图集纹素中计算的，并非关于摄像机所见内容的声明**：膨胀占已渲染图集的26.95%，占渲染像素的4.95%，比例为0.18。颜色存在于大型图表中，孔洞存在于小型图表中，因此在屏幕空间中，膨胀纹素的成本较低。
- **⚑ 决定接受标准的缺陷是由颜色引起的，而不是由任何填充引起的**——即显示另一种材质颜色的区域，而斑点统计无法检测到这些区域。通过三种方式、在三个不同的空间中进行了三次测量：**91.05%的缺陷与`reference`相关，并且具有0.99倍的增强效果**，这与基准率非常接近；相同类别的绿色布料为**68.46% `reference`**；而在薄刀片上，表面本身的彩色纹素有**18.77%**受到污染，而与其膨胀填充相比，该比例为**5.55%**。填充正确地从其最近的彩色邻居获取颜色——而这个邻居本身就存在问题。混合本身是一种未记录的双频带分割（`M + gaussian_blur_σ16(B − M)`），它在相同的点上测量了四个替代方案中最差的结果。
- **各个视图并非相互独立，这限制了所有混合修复的可能性。**对于每个缺陷斑块，**100%的具有两个或多个参与摄像机的面都位于90°范围内**（平均为45°），并且21%的缺陷面仅由一个摄像机观察到。在近乎相同的控制下，相邻视图会同时出现故障，因此摄影测量学中发布的基于多视点的优势无法直接应用于此处。
- **此渲染流程中的每次重建都是一个空心双层外壳**，壁厚约为两个体素。没有体积谓词可以对其进行验证。
- **图像在未命名的材质边界处存在差异，而标准构建至关重要**（2026年8月16日）。内部与网格对齐的扭曲测量结果显示，所有八个视图中，与轮廓的中值相比，平均值为**3.5–11.1像素**；导演圈出的每个残余区域——袖口、手部、鞋面顶部——都是材质连接点，而生成提示中未提及这些连接点（记录的提示包含六个元素；抓握、护腕、胫甲和手部出现**零**次）。他的诊断结果是：“我们没有正确地构建标准。”W3标准构建和基于标准的双重再生是计划中的修复方案（[E49分发记录](docs/experiments/E49-finish-and-cap-kickoff.md））
- **5.57%–4.65%的有效纹素是任何平面环形摄像机都无法看到的表面**——它们在每个视图中均未能通过深度门，没有任何投影渲染流程可以渲染这些纹素，并且已发布的渲染流程使用岛屿盲区填充来覆盖这些区域，从而导致出现深色标记。它们需要一种策略（中性材质、画笔或接受），而不是修复方案（[E49报告](docs/experiments/E49-finish-and-cap-report.md））
- **完整的候选填充渲染会呈现出平坦的彩色多边形**——这是导演在已接受等级的图像中提出的唯一问题（“看起来很棒，但存在着有颜色的多边形”）。正在测试的假设是，已经标记了来源掩码：孤立岛屿的大小与单个三角形相同，并且从边界相邻的双重样本中进行平坦填充，这些样本使用未侵蚀的轮廓。
...
- **测试与修改代码相关的提交版本相关联**——在两次操作中，有 1233 个通过测试，并且对 1181 个经过严格隔离的版本进行了路径限制的 CI 测试。
...
CI 在 **ubuntu-latest / Python 3.12** 上运行套件的封闭子集，并进行固定安装（`.github/workflows/ci.yml`）；工件层需要记录在 `E:\AI\training` 下的树，这些内容不在 git 中，因此 CI 会按设计选择不包含它们。本地，`python -m pytest` 运行所有 **1233** 个测试，而 `python -m pytest -m "not artifacts"` 运行 CI 重现的 **1181** 个测试。
