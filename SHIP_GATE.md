...
- [x] `[all]` `verify` script exists (test + build + smoke in one command) (2026-08-08) — `python -m pytest` runs the full **202-test** suite; `python -m pytest -m "not artifacts"` runs the **194** hermetic tests CI reproduces. There is no build step because nothing is built. The artifacts tier is the smoke layer — it replays recorded trees and anchors. Configured in `pytest.ini`, gated in `.github/workflows/ci.yml`, established in [E17 Ruling 5](docs/experiments/E17-ruling.md) and extended by [E18](docs/experiments/E18-ruling.md). *Re-counted 2026-08-08 at the E19 ruling: this line read 32/24, true when written and stale the same day when E18 landed 60 tests in the parallel lane. It was then corrected to 92/84 and went stale **again within the ruling session** when E20 committed its unit tier. Lineage 27 → 32 → 92 → 202 in one day. **This figure is provisional while E20 is live** and is re-counted at the tagging commit, per [E19 Ruling 7](docs/experiments/E19-ruling.md).*
