...
- **Testes acompanham o commit que afeta o código** — 1375 aprovados por duas pessoas, com CI baseado em caminhos nos 1304 experimentos herméticos.
...
O CI executa o subconjunto hermético da suíte em **ubuntu-latest / Python 3.12** com instalações fixas (`.github/workflows/ci.yml`); a camada de artefatos precisa das árvores registradas sob `E:\AI\training`, que não estão no git, então o CI as desativa por design. Localmente, `python -m pytest` executa todos os **1375** testes e `python -m pytest -m "not artifacts"` executa os **1304** testes reproduzidos pelo CI.
