...
O estilo é aplicado **no objeto**, no espaço da textura — não é pintado para cada perspectiva e depois combinado. Forneça ao programa um modelo conceitual em argila com formas exageradas e ele retornará uma malha texturizada cuja cor foi obtida a partir de uma referência estilizada *dessa* mesma malha, preenchendo tudo o que a referência não conseguiria visualizar com um pincel de retoque mascarado e uma dilatação consciente da superfície.

O nome faz referência a ambas as partes do problema: os polígonos e a superfície que eles devem preencher.
...
- **Priorize a forma, depois o estilo.** Os programas de reconstrução interpretam o ruído superficial como geometria. Uma argila limpa, com aparência escultórica e planos deliberadamente exagerados, resulta numa topologia melhor do que um sprite estilizado; o modelo estilizado é gerado em paralelo e serve como referência de cor.
...
- **A identidade pertence à instrução.** Um elemento canônico não mencionado na instrução aparece por acaso e desaparecerá da mesma forma – medido quando as placas douradas nos joelhos acabaram aparecendo na imagem apenas através do ruído numa versão corrompida do ControlNet.
- **Consulte a geometria, não um limite.** Substituir uma máscara com o contorno exato obtido pelo raycasting moveu a cobertura de referência de **28,4% para 39,1%** de texels válidos – estritamente aditivo, sem difusão, sem uso da GPU. O método de seleção por mediana nos cantos falhou três vezes aqui e foi descontinuado.
...
Identificados e descritos, na página principal em vez de numa nota de rodapé. [Todos eles estão listados no código](docs/known-defects.md).

- **A faixa da lâmina representa 0,00% da referência do estágio 1** em todas as oito câmaras — o aço sobre um fundo cinzento está posicionado exatamente no limite definido. A união resgata 55,72%.
...
- **Os erros permanecem no repositório com a sua respectiva razão.** [`tools/superseded/`](docs/tools.md) não é um arquivo — qualquer pessoa pode executar essas ferramentas e observar o mesmo resultado negativo.
- **Um resultado negativo é um sucesso completo**, relatado e encerrado, em vez de ajustado para atingir um determinado valor.
- **Os testes acompanham o commit que modifica o código** — 202 aprovados por duas pessoas, com CI restrito a caminhos nos 194 casos herméticos.
- **O registro pode ser consultado.** Um índice SQLite + FTS5 sobre todo o histórico, verificado em quatro pontos. Ele encontrou uma contagem que diferia do que estava escrito no texto em três locais, contando o próprio registro.
...
- **Dados acessados:** malhas, texturas, imagens e JSON no disco local, nos caminhos que
você passa na linha de comando. Além disso, `docs/index/facet.db`, que é *derivado* — ele contém
...
- **Sem telemetria.** Nenhuma coletada, nenhuma enviada. Não há opção de desativar porque não há
nada para desativar.
...
**Status de suporte:** este repositório é desenvolvido em código aberto, em um único ambiente, por um diretor
e uma dupla rotativa de sessões de consultor e executor. `main` é o único estado suportado. Não há canal de lançamento, nenhuma política de retrocompatibilidade e nenhum SLA — em vez disso, existe
...
Localmente, `python -m pytest` executa todos os **202** testes e `python -m pytest -m "not artifacts"`
executa os **194** que o CI reproduz.
