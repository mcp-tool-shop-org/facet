...
- **Os testes acompanham o commit que toca o código** — 1342 aprovados por duas pessoas, com CI baseado em caminhos nos 1288 casos herméticos.
...
O CI executa o subconjunto hermético do conjunto de testes em **ubuntu-latest / Python 3.12** com instalações fixas (`.github/workflows/ci.yml`); a camada de artefatos precisa das árvores registradas sob `E:\AI\training`, que não estão no git, portanto, o CI as desativa por design. Localmente, `python -m pytest` executa todos os **1342** testes e `python -m pytest -m "not artifacts"` executa os **1288** testes reproduzidos pelo CI.
