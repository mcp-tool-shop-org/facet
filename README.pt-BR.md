...
número.
- **Os testes acompanham o commit que toca o código** — 891 aprovados por duas pessoas, com CI com restrições de caminho nos 851 herméticos.
...
O CI executa o subconjunto hermético da suíte no ambiente **ubuntu-latest / Python 3.12**, com instalações fixas (`.github/workflows/ci.yml`); a camada de artefatos necessita das árvores registradas em `E:\AI\training`, que não estão no Git, portanto, o CI as exclui intencionalmente. Localmente, `python -m pytest` executa todos os **891** testes e `python -m pytest -m "not artifacts"` executa os **851** testes reproduzidos pelo CI.
