...
| # | element | note |
|---|---|---|
| D1 | deep moss-green scaled hide | the dominant material: back, flanks, neck top, limbs, tail top, and the wing arms and fingers. Sits in the style's proven green register (W3's tunic band) — the LoRA has painted this family before. *⚠ Term added 2026-08-06 (E12 Ruling 17e, the Director's eye on the twin set): the wing arms and fingers were assigned here but never NAMED in the prompt string — and on the two wing-spread views (0, 4) the unclaimed skeleton went bone-ivory (the horns' resemblance channel + the bat-anatomy prior), while the folded-wing views kept it green by context. The founding law, measured again: a surface not named in the prompt is painted by accident in both directions. The protective entry now carries `moss-green wing arms and finger struts` as its own noun phrase; occupancy unchanged — the surface was always D1's* |
...
| D3 | leathery storm-grey wing membranes | the sheet fields between the wing fingers, vein relief included. Deliberately cool and neutral against the warm hide — and deliberately NOT the studio-grey of a clay backdrop; the backdrop derivation (S-backdrop) must hold this element furthest of the large surfaces. *⚠ Term strengthened 2026-08-06 (E12 Ruling 22b, the Director's pre-stage-1 iteration): four generations of measured drift — ivory under family pressure, slate-to-cream lit gradient, orange at bust frame, warm tan across the v8 set — point at the realistic register's translucency prior (backlit membrane tissue renders as warm glow), and a colour word alone does not contest a MATERIAL prior. "Leathery" adds the opacity cue; the hue family is unchanged, so the bands ruling and backdrop arithmetic are untouched* |
