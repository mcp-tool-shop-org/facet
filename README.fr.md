...
- **Les tests sont liés au commit qui touche le code** — 1 182 tests réussis, évalués par deux personnes, avec des CI à accès limité sur les 1 135 tests hermétiques.
...
L’intégration continue (CI) exécute l’ensemble hermétique de la suite sur **ubuntu-latest / Python 3.12** avec des installations fixes (`.github/workflows/ci.yml`) ; le niveau d’artefacts a besoin des arbres enregistrés sous `E:\AI\training`, qui ne sont pas dans Git, donc la CI les exclut intentionnellement. Localement, `python -m pytest` exécute les **1 182** tests et `python -m pytest -m "not artifacts"` exécute les **1 135** tests reproduits par la CI.
