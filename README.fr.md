...
- **Les tests sont liés au commit qui modifie le code** — 1166 tests réussis, avec des contrôles d’intégration basés sur les chemins pour les 1121 tests hermétiques.
...
Le CI exécute l’ensemble hermétique de la suite sur ubuntu-latest / Python 3.12 avec des installations fixes (`.github/workflows/ci.yml`) ; le niveau des artefacts nécessite les arbres enregistrés sous `E:\AI\training`, qui ne se trouvent pas dans git, de sorte que le CI les désactive intentionnellement. Localement, `python -m pytest` exécute les 1 166 tests et `python -m pytest -m "not artifacts"` exécute les 1 121 tests reproduits par le CI.
