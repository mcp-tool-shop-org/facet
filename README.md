...
- **A negative result is a full success**, reported and closed rather than tuned toward a
  number.
- **Tests ride the commit that touches the code** — 790 passing at two seats' hands, with
  paths-gated CI on the 761 hermetic ones.
...
Locally, `python -m pytest` runs all **790** tests and `python -m pytest -m "not artifacts"`
runs the **761** CI reproduces.
