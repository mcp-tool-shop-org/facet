...
- **A negative result is a full success**, reported and closed rather than tuned toward a
  number.
- **Tests ride the commit that touches the code** — 1295 passing at two seats' hands, with
  paths-gated CI on the 1241 hermetic ones.
...
Locally, `python -m pytest` runs all **1295** tests and `python -m pytest -m "not artifacts"`
runs the **1241** CI reproduces.
