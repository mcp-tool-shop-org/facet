...
- **A negative result is a full success**, reported and closed rather than tuned toward a
  number.
- **Tests ride the commit that touches the code** — 1252 passing at two seats' hands, with
  paths-gated CI on the 1198 hermetic ones.
...
Locally, `python -m pytest` runs all **1252** tests and `python -m pytest -m "not artifacts"`
runs the **1198** CI reproduces.
