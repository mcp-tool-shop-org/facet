...
- **Spec before the work, report after, ruling last** — and the session that designs an
  experiment never grades its own results. Twenty-six experiments are in
...
- **A negative result is a full success**, reported and closed rather than tuned toward a
  number.
- **Tests ride the commit that touches the code** — 684 passing at two seats' hands, with
  paths-gated CI on the 675 hermetic ones.
...
| **[The record](docs/experiments/)** | twenty-six experiments: spec, report, ruling, and every prediction stated before the measurement |
...
Locally, `python -m pytest` runs all **684** tests and `python -m pytest -m "not artifacts"`
runs the **675** CI reproduces.
