...
- **Spec before the work, report after, ruling last** — and the session that designs an
  experiment never grades its own results. Thirty-nine experiments are in
...
- **A negative result is a full success**, reported and closed rather than tuned toward a
  number.
- **Tests ride the commit that touches the code** — 1072 passing at two seats' hands, with
  paths-gated CI on the 1027 hermetic ones.
...
| **[The record](docs/experiments/)** | thirty-nine experiments: spec, report, ruling, and every prediction stated before the measurement |
...
Locally, `python -m pytest` runs all **1072** tests and `python -m pytest -m "not artifacts"`
runs the **1027** CI reproduces.
