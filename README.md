...
- **Spec before the work, report after, ruling last** — and the session that designs an
  experiment never grades its own results. Thirty-one experiments are in
...
- **A negative result is a full success**, reported and closed rather than tuned toward a
  number.
- **Tests ride the commit that touches the code** — 924 passing at two seats' hands, with
  paths-gated CI on the 884 hermetic ones.
...
| **[The record](docs/experiments/)** | thirty-one experiments: spec, report, ruling, and every prediction stated before the measurement |
...
Locally, `python -m pytest` runs all **924** tests and `python -m pytest -m "not artifacts"`
runs the **884** CI reproduces.
